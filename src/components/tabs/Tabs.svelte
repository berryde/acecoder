<script lang="ts">
	import { openTab, rearrange } from '../../utils/tabs/tabs';
...
<div class="flex flex-row bg-brand-background  w-full items-center">
	{#each tabs as path}
		<Draggable data={path} variant="tabs">
			<Droppable let:dropping on:dropped={(e) => handleDropped(path, e.detail)} variant="tabs">
				<Hoverable let:hovering>
					<div
						class="text-brand-text max-w-min transition-colors pl-4 pr-2 py-1 selected flex flex-row space-x-1 items-center {path ==
						selected
							? 'bg-brand-editor-background'
							: 'hover:bg-brand-accent'}  duration-75 cursor-default {dropping &&
